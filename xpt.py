...
# Robust playlist fetch (m3u) with progress
def fetch_playlist_robust(server_url, username, password, m3u_type="m3u_plus", timeout=DEFAULT_TIMEOUT, verbose=True):
    ...
    for base in endpoints:
        ...
        if status != 200:
            ...
            # try to save snippet if available
            try:
                text_snip = resp.text
            ...
            for chunk in resp.iter_content(chunk_size=8192):
                ...
                if verbose:
                    if total:
                        ...
                        sys.stdout.write(f"\r  Downloaded: {downloaded}/{total} bytes ({pct:5.1f}%)")
                    else:
...
def parse_m3u_to_json(m3u_text):
    ...
    while i < len(lines):
        ...
        if ln.upper().startswith("#EXTINF"):
            ...
            # format: #EXTINF:-1 tvg-id="..." tvg-name="..." ,Channel Title
            # split header and title by the first comma
            parts = ln.split(",", 1)
            if len(parts) == 2:
                header, title = parts[0], parts[1].strip()
            else:
                header, title = parts[0], ""
            # duration after colon
            dur_match = re.match(r'#EXTINF:([-0-9]+)', header, re.IGNORECASE)
            duration = dur_match.group(1) if dur_match else ""
            # attributes: find key="value" pairs
...
def save_playlist_json(safe_name, username, channels):
    ...
    try:
        with open(path, "w", encoding="utf-8") as fh:
            json.dump(channels, fh, indent=2, ensure_ascii=False)
    except Exception as e:
...
def build_extinf_line(entry):
    ...
    # build attrs string like: tvg-id="..." tvg-name="..." group-title="..."
    attr_parts = []
    for k, v in attrs.items():
        # ensure quotes and escape any double quotes inside value
        safe_v = v.replace('"', "'")
...
def fetch_and_save_playlist(idx):
    ...
    with open(path, "w", encoding="utf-8") as f:
        f.write(text)
    # parse and save JSON automatically
    channels = parse_m3u_to_json(text)
    ...
    while True:
        ...
        if choice == "1":
            ...
            try:
                ...
                channels = parse_m3u_to_json(text)
                # derive safe_name/username from filename if possible, else generic
            ...
            try:
                ...
                if not out_name:
                    base = os.path.splitext(file)[0]
                    out_name = f"{base}_{field}-{keyword or 'all'}_filtered"
                ...
                if ok:
                    ...
                    # also save as JSON
                    try:
                        # attempt to save JSON using same name
            ...
            try:
                ...
                # show sample entries count and option to print first N
                sample_n = int(input("How many sample entries to show? (0 to cancel): ").strip() or "0")
                if sample_n > 0:
                    for i, ch in enumerate(channels[:sample_n], 1):
